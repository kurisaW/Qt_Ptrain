...
void MainWindow::onNewConnection(qintptr socketDes)
{
    ...
    connect(thread,SIGNAL(sigUserSendMsg(UserData,UserData,QString)),this,SLOT(onUserSendMsg(UserData,UserData,QString)));
...
{
    if(users.count(to) == 0)//接收消息的用户不在线就不转发，此处可用于离线消息，需处理，否则会导致服务器崩溃
        return ;

    //users[to]根据to这个键找到对应的值，即线程对象的指针
